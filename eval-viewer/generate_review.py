...
def build_run(root: Path, run_dir: Path) -> dict | None:
    ...
    # Normalize grading format: skill-optimizer uses "checklist_results" with
    # "item" keys, but the viewer expects "expectations" with "text" keys.
    if grading and "checklist_results" in grading and "expectations" not in grading:
        grading["expectations"] = [
            ...
            for r in grading["checklist_results"]
        ]
...
def embed_file(path: Path) -> dict:
    ...
    if file_size > MAX_EMBED_SIZE:
        return {
            ...
            "mime": mime,
            "data_uri": "#",
            "content": f"(File too large to embed: {file_size / 1024 / 1024:.1f} MB. Max: {MAX_EMBED_SIZE / 1024 / 1024:.0f} MB)",
        }
